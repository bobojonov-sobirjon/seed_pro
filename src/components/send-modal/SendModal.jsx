...
function SendModal({ data, isOpen, onClose = true }) {
  ...
  return (
    <>
      <Dialog placeholder={<div />} open={isOpen} handler={onClose} className='lg:p-8 w-[96%] lg:w-[500px]' size='lg'>
        <DialogHeader placeholder={<div />} className='uppercase flex items-center lg:gap-2 flex-wrap text-[16px] lg:text-[26px]'>
          <span className='pr-2'>
        ...
        <div className='flex flex-wrap gap-2 p-4 w-[300px]'>
          <div className='bg-white shadow rounded w-full'>
            ...
            >
              ...
              {projects.map((tech) => {
                return <option key={Math.random().toString()} value={tech?.id}>{tech?.name}</option>;
              })}
            </select>
          </div>
        </div>
