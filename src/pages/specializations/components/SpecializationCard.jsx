  ...
  return (
    <>
      {data && data?.results && Array.isArray(data.results) && data.results?.length > 0 ? data.results.map(item => (
        <div key={Math.random().toString()} className='flex items-start p-6 gap-4 bg-white shadow-lg rounded-lg cursor-pointer' onClick={() => navigateHandler(item)}>
        ...
        <div className='flex justify-center mt-4'>
          <span className='text-gray-500'>{t("extraComponents.specializationCard.not_found")}</span>
        </div>
      )}
