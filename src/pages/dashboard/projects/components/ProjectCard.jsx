...
const ProjectCard = (props) => {
  ...
  const navigateHandler = item => {
    navigate('/admin/projects/details/', { state: item });
  }
  ...
  return (
    data?.length > 0 && data.map(item => {
      ...
      return (
        <div key={item.id} className='bg-white p-4 lg:p-8 shadow-xl rounded-lg mb-6 flex flex-col md:flex-row justify-between lg:items-center'>
          <div className='flex flex-1 items-center relative'>
            <div className='flex items-center justify-center h-[50px] lg:h-20 w-[50px] lg:w-20 rounded-full mr-2 lg:mr-4'>
              <img src={item.project_image} alt="no image" className='w-full h-full rounded-full' />
            </div>
