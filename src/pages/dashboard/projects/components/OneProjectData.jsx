    ...
    state.komanda.map(
      (elem) =>
        ...
        elem.map((item) => (
          <div
            key={item.id}
            className="p-2 lg:p-6 bg-white shadow-lg rounded-lg"
          >
            ...
            <div className="flex flex-col gap-2 mt-2">
              <div className="">
                <h3 className="font-gilroy-bold text-custom-gray text-sm">
                  ...
                  <span className="text-gray-600">{item.tasks}</span>
                </h3>
              </div>
              <div className="">
                <h3 className="font-gilroy-bold text-sm text-custom-gray">
                  ...
                  {item.tags?.length > 0 &&
                    item.tags.map((el, index) => (
                      <span key={el.id} className=" text-gray-500">
                        {el.name} {item.tags?.length - 1 !== index && ","}
                      </span>
                    ))}
                </h3>
              </div>
