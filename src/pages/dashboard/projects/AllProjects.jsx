...
import { getAllAbout, getAllCourses, getAllEducations, getAllExperience, getProfile } from '../../../redux/reducers/profileReducer';
...
const AllProjects = (props) => {
  ...
  useEffect(() => {
    ...
    props.onGetAllPorjects({ signal, selectedId: selected + 1, limit: 15, komanda_position: "", komanda_level: "" }); // get profile
  ...
  const handlePageClick = async (e) => {
    ...
    props.onGetAllPorjects({ selectedId: e.selected + 1, limit: 15, komanda_position: position_ref.current?.value, komanda_level: level == -1 ? "" : level }); //get all data
  }
  ...
  const searchElementsHandler = () => {
    if (position_ref.current?.value || level) {
      ...
      props.onGetAllPorjects({ selectedId: selected + 1, limit: 15, komanda_position: position_ref.current?.value, komanda_level: level == -1 ? "" : level }); //get all data
    ...
    <div className='max-w-5xl mx-auto my-10 mb-2'>
      <main>
        {props.loading ? (
          ...
          (props.profileData && props.profileData?.information?.length > 0 && props.allExperience?.length > 0 && props.allAbout?.length > 0 && !props.profileData.active) ? (
            <div>
              <>
                ...
                <div className='flex lg:flex-row flex-col lg:gap-0 gap-2 lg:items-center space-x-4 py-8 pt-4'>
                  <div className='gap-3 grid grid-cols-1 lg:grid-cols-10'>
                    ...
                    <button className='col-span-1 p-3 px-10 bg-custom-gray text-white rounded-md lg:col-span-2' onClick={searchElementsHandler}>
