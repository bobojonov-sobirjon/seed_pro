...
const Messages = (props) => {
  ...
  const navigateHandler = item => {
    navigate("/admin/specializations/specialization-detail", { state: { ...item, isChange: false } });
  }
  return (
    <div className='h-screen w-full lg:max-w-5xl mx-auto my-4 lg:my-10 grid grid-cols-7 gap-4'>
      <div className={`relative max-h-[calc(100vh-160px)] col-span-7 lg:col-span-3 rounded-lg ${selectedUser.hasOwnProperty('id') ? 'hidden lg:block' : 'block lg:block'}`}>
        <form onSubmit={getUser} className='flex items-center gap-2 mb-2 lg:mb-4 w-full'>
          ...
          <button type='submit' className={`${styles.active} h-[50px] rounded-xl border bg-text-main_green active:bg-black active:transition-all active:duration-500 flex items-center justify-center cursor-pointer`}>
          ...
          <div className='user_sidebar overflow-y-scroll h-[calc(100vh-160px)] lg:h-[calc(100vh-160px)]'>
            {isUserLoading ? (
              ...
              <Users
                ...
                removeNotificationWithSelectedUser={removeNotificationWithSelectedUser}
              />
            )}
          </div>
        </div>
      ...
      {selectedUser.hasOwnProperty("id") && (
        <div
          className={`max-h-[calc(100vh-160px)] overflow-y-auto col-span-7 lg:col-span-4 bg-white rounded-lg shadow ${selectedUser.hasOwnProperty('id') ? 'flex' : 'hidden'
        ...
        >
            ...
            <div className="flex items-center w-full gap-4">
              ...
              <img
                src={
                  selectedUser?.receiver?.avatar ?
                    selectedUser.initiator.avatar : "https://github.com/shadcn.png"}
              ...
              <div className="text-sm lg:text-base">
                {selectedUser?.receiver?.id !== props.profileData.id ? (
                  <h5
                    onClick={() =>
                      navigateHandler(selectedUser.receiver || selectedUser.initiator)
                    }
                    className="cursor-pointer font-semibold leading-[19.6px] text-[16px] font-montserrat"
                  >
                    {selectedUser.receiver?.first_name || selectedUser.initiator?.first_name}{' '}
                    {selectedUser.receiver?.last_name || selectedUser.initiator?.last_name}
                  </h5>
                ) : (
                  <h5
                  ...
                  >
                    {selectedUser.initiator?.first_name} {selectedUser.initiator?.last_name}
                  </h5>
                )}
              </div>
            </div>
          ...
          {true? (
            <div className="flex items-center bg-white border-t p-4 absolute bottom-0 left-0 right-0">
              <input
                type="text"
                placeholder={t('dashboard.messages.enter_message_input_placeholder')}
