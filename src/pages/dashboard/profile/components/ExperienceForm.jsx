...
function ExperienceForm({
  ...
  useEffect(() => {
    if (end_date_ref.current) {
      end_date_ref.current.disabled = isChecked;
      end_date_ref.current.required = !isChecked;
    }
  }, [isChecked, end_date_ref]);
  return (
    <form onSubmit={handleSubmit((data) => handleExperience(data))}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <span className="block mb-2 text-[13px] lg:text-sm font-bold text-custom-gray">
            {t("dashboard.profile.experience.job_title_input_label")}{" "}
            <span className="text-red-700">*</span>
          </span>
          <input
            className={`bg-gray-50 text-gray-900 text-sm rounded-lg outline-none border focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5 ${
              errors.position ? "border-red-700" : "border-gray-50"
            }`}
        ...
        <div>
          <span className="block mb-2 text-[13px] lg:text-sm font-bold text-custom-gray">
            {t("dashboard.profile.experience.company_input_label")}{" "}
            <span className="text-red-700">*</span>
          </span>
          <input
            className={`bg-gray-50 text-gray-900 text-sm rounded-lg outline-none border focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5 ${
              errors.company ? "border-red-700" : "border-gray-50"
            }`}
        ...
        <div>
          <span className="block mb-2 text-[13px] lg:text-sm font-bold text-custom-gray">
            {t("dashboard.profile.experience.start_work_time_label")}{" "}
            <span className="text-red-700">*</span>
          </span>
          <input
            type="date"
        ...
        <div>
          <span className="block mb-2 text-[13px] lg:text-sm font-bold text-custom-gray">
            {t("dashboard.profile.experience.finish_work_time_label")}{" "}
            <span className="text-red-700">*</span>
          </span>
          <input
            type="date"
            name="end_date_experience"
            className="relative block w-full appearance-none rounded-md p-2.5 bg-custom-light placeholder-gray-500 focus:z-10 focus:outline-none text-sm cursor-pointer disabled:cursor-not-allowed disabled:bg-gray-300"
            ref={end_date_ref}
            required={!isChecked}
            disabled={isChecked}
            autoComplete="true"
          ...
          <div className="flex items-center mt-2">
            <label
              ...
              className="flex items-center cursor-pointer current_experience"
            >
              <input
      ...
      <div className="grid grid-cols-2 lg:flex lg:items-center lg:justify-end gap-4 mt-2">
        ...
        <button
          type="submit"
          className={`text-text-main_green border col-span-1 border-text-main_green transition-all hover:bg-main-green hover:text-white font-gilroy-bold py-3 px-[35px] rounded focus:outline-none text-[13px] focus:shadow-outline ${
